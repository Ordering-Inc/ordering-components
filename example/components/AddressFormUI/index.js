...
export const AddressFormUI = (props) => {
  ...
  const inputs = [
    // { name: 'address', placeholder: 'Address', requiredMessage: 'Address is required', type: 'text' },
    {
      ...
      requiredMessage: 'Address notes are required',
      type: 'textarea'
    },
    {
      name: 'internal_number',
      placeholder: 'Internal number',
      requiredMessage: 'Internal number is required',
      type: 'text'
    },
    {
      name: 'zipcode',
  ...
  const handleAddressKeyDown = () => {
    state.selectedFromAutocomplete &&
      setState({
        ...state,
      ...
      {!validationFields.loading && !addressState.loading && (
        <>
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            />
            {errors.address && (
              <i style={{ color: '#c10000' }}>{errors.address.message}</i>
            )}
            {!state.selectedFromAutocomplete && (
              <i style={{ color: '#c10000' }}>
                Select address from autocomplete.
              </i>
            )}
            <GoogleGpsButton apiKey='' onAddress={handleChangeAddress} />
            {inputs.map((_input) => (
                ...
                {showField(_input.name) && _input.type === 'text' && (
                  <div>
                    <input
                      ...
                      ref={register({
                        required: isRequiredField(_input.name)
                          ? _input.requiredMessage
                          : null
                      })}
                    />
                    {errors[_input.name] && (
            ...
            {!formState.loading && formState.error && (
              <p style={{ color: '#c10000' }}>{formState.error}</p>
            )}
