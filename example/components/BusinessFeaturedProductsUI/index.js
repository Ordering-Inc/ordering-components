            ...
            <div style={{ display: 'flex', flexWrap: 'wrap' }}>
              {productsList.products && productsList.products.length > 0 ? (
                productsList.products.map(product => (
                  <div
                    key={product.id}
                    style={{ border: '1px solid gray', width: '300px', padding: '10px', marginBottom: '10px', display: 'flex', justifyContent: 'space-between', marginRight: '10px' }}
                    onClick={() => handlerClickProduct(product.id)}
                  >
