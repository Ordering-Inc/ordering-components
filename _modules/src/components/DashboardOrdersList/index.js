...
var DashboardOrdersList = function DashboardOrdersList(props) {
  var _configState$configs$, _paginationSettings$p;
  ...
  var getTaxPrice = function getTaxPrice(order, subTotalPrice) {
    ...
    if (order.tax_type === 2) {
      if (order.discount > 0) {
        taxPrice = (subTotalPrice - order.discount) * (order === null || order === void 0 ? void 0 : order.tax) / 100;
      } else {
        taxPrice = subTotalPrice * (order === null || order === void 0 ? void 0 : order.tax) / 100;
      }
    }
  ...
  var getServiceFee = function getServiceFee(order, subTotalPrice) {
    var serviceFee = 0;

    if (order.service_fee > 0) {
      if (order.discount > 0) {
        serviceFee = (subTotalPrice - order.discount) * (order === null || order === void 0 ? void 0 : order.service_fee) / 100;
      } else {
        serviceFee = subTotalPrice * (order === null || order === void 0 ? void 0 : order.service_fee) / 100;
      }
    }
  ...
  var getProductPrice = function getProductPrice(product) {
    ...
    var productPrice = product.quantity * (product.price + subOptionPrice);
    return parseFloat(productPrice.toFixed(decimal));
  };

  var getSubTotalPrice = function getSubTotalPrice(order) {
    var orderSubTotalPrice = 0;

    var _iterator3 = _createForOfIteratorHelper(order.products),
        _step3;
  ...
  var getTotalPrice = function getTotalPrice(order) {
    ...
    if ((order === null || order === void 0 ? void 0 : order.driver_tip) > 0) {
      orderTotalPrice += subTotalPrice * order.driver_tip / 100;
    }

    if ((order === null || order === void 0 ? void 0 : order.discount) > 0) {
